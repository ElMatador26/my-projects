# -*- coding: utf-8 -*-
"""
Created on Wed Aug 16 18:36:16 2023
    ...
    def distance(self, col: str = 'name'):
        ...
        for i in range(self.df.shape[0]):
            for j in range(i, self.df.shape[0]):
                mat[i,j]=(nltk.edit_distance(self.df.loc[i, col], self.df.loc[j,col]))
    ...
    def __create_coord_dict(self, mat):
        ...
        while l != j:
            ...
            if mat.row[i] in dict1 and mat.col[i] not in dict1:
                dict1[mat.col[i]] = self.__coord_generator(dict1[mat.row[i]], mat.data[i])
                j+=1
            if mat.row[i] not in dict1 and mat.col[i] in dict1:
                dict1[mat.row[i]] = self.__coord_generator(dict1[mat.col[i]], mat.data[i])
                j+=1
